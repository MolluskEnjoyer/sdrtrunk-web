...
public class DecoderApiHandler extends BaseApiHandler
{
    ...
    private void handleStart(HttpExchange exchange) throws IOException
    {
        ...
        try
        {
            ...
            // Attach event listener to processing chain for live activity logging
    ...
    private void handleStatus(HttpExchange exchange) throws IOException
    {
        ...
        // Live channel info from the metadata model
        if(mDecoderRunning && mWebChannel != null)
        {
            result.addProperty("channel_name", mWebChannel.getName());

            // Scan all active channel metadata rows for the most interesting state
            ChannelMetadataModel metaModel = mPlaylistManager.getChannelProcessingManager()
                .getChannelMetadataModel();

            String activeTg = null;
            String activeTgAlias = null;
            String activeFrom = null;
            String activeFromAlias = null;
            Long activeFreq = null;
            String activeState = null;
            String activeChannel = null;

            for(int i = 0; i < metaModel.getRowCount(); i++)
            {
                ChannelMetadata meta = metaModel.getChannelMetadata(i);
                if(meta == null) continue;

                String state = meta.getChannelStateIdentifier() != null
                    ? meta.getChannelStateIdentifier().toString() : "IDLE";

                // Prefer CALL > ACTIVE > DATA > CONTROL > anything else
                boolean isBetter = activeState == null ||
                    ("CALL".equals(state) && !"CALL".equals(activeState)) ||
                    ("ACTIVE".equals(state) && !"CALL".equals(activeState) && !"ACTIVE".equals(activeState));

                if(isBetter)
                {
                    activeState = state;

                    if(meta.hasToIdentifier())
                    {
                        activeTg = meta.getToIdentifier().toString();
                    }
                    if(meta.getToIdentifierAliases() != null && !meta.getToIdentifierAliases().isEmpty())
                    {
                        activeTgAlias = meta.getToIdentifierAliases().get(0).getName();
                    }
                    if(meta.hasFromIdentifier())
                    {
                        activeFrom = meta.getFromIdentifier().toString();
                    }
                    if(meta.getFromIdentifierAliases() != null && !meta.getFromIdentifierAliases().isEmpty())
                    {
                        activeFromAlias = meta.getFromIdentifierAliases().get(0).getName();
                    }
                    if(meta.hasFrequencyConfigurationIdentifier())
                    {
                        activeFreq = meta.getFrequencyConfigurationIdentifier().getValue();
                    }
                    if(meta.hasDecoderLogicalChannelNameIdentifier())
                    {
                        activeChannel = meta.getDecoderLogicalChannelNameIdentifier().getValue();
                    }
                }
            }

            if(activeTg != null) result.addProperty("active_tg", activeTg);
            if(activeTgAlias != null) result.addProperty("active_tg_alias", activeTgAlias);
            if(activeFrom != null) result.addProperty("active_from", activeFrom);
            if(activeFromAlias != null) result.addProperty("active_from_alias", activeFromAlias);
            if(activeFreq != null) result.addProperty("active_freq", activeFreq);
            if(activeState != null) result.addProperty("active_state", activeState);
            if(activeChannel != null) result.addProperty("active_channel", activeChannel);
        }
    ...
    /**
     * Attach a DecoderStateEvent listener to the processing chain for the given channel.
     * This logs call activity (CALL start/end, state changes) to the activity log.
     */
    private void attachEventListener(Channel channel)
    {
        try
        {
            ChannelProcessingManager cpm = mPlaylistManager.getChannelProcessingManager();
            ...
            if(chain != null)
            {
                chain.addDecoderStateEventListener(new Listener<DecoderStateEvent>()
                {
                    ...
                    public void receive(DecoderStateEvent event)
                    {
                        ...
                        if(newState == mLastState) return;

                        State oldState = mLastState;
                        mLastState = newState;

                        switch(newState)
                        {
                            case CALL:
                                long freq = event.getFrequency();
                                String freqStr = freq > 0 ? String.format("%.4f MHz", freq / 1e6) : "";
                                addLog("CALL START" + (freqStr.isEmpty() ? "" : " - " + freqStr));
                                break;
                            case FADE:
                            case TEARDOWN:
                                if(oldState == State.CALL || oldState == State.ACTIVE)
                                {
                                    addLog("CALL END");
                                }
                                break;
                            case ACTIVE:
                                addLog("ACTIVE - Traffic channel");
                                break;
                            case CONTROL:
                                // Only log once at startup
                                if(oldState == null || oldState == State.IDLE)
                                {
                                    addLog("CONTROL channel acquired");
                                }
                                break;
                            default:
                                break;
        ...
        {
            mLog.error("Failed to attach event listener to processing chain", e);
        }
    }
